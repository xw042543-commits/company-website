...
export function FilterPanel({ locale, query, directory = false }: { locale: Locale; query: Query; directory?: boolean }) {
  ...
  return <aside className="filter-panel">
    <button type="button" className="mobile-filter-toggle" aria-expanded={expanded} aria-controls="filter-fields" onClick={() => setExpanded(!expanded)}>{words(locale, "筛选条件", "Filters")} {expanded ? "−" : "+"}</button>
    <div id="filter-fields" className={expanded ? "filter-fields expanded" : "filter-fields"}>
      ...
      <form action={path} method="get" key={`${locale}:${JSON.stringify(query)}`}>
        {!directory && <><div className="field"><label htmlFor="q">{words(locale, "专业关键词", "Course keyword")}</label><input id="q" name="q" type="search" maxLength={100} defaultValue={first(query, "q")} /><small>{words(locale, "正式搜索将支持老板确认的别名。", "Search will support owner-approved aliases.")}</small></div>
          {select("category", "专业分类 / 领域", "Subject category / field", [])}<small>{words(locale, "分类选项待提供。", "Category options are awaiting approval.")}</small>
        ...
        {!directory && <>
          ...
          {select("language", "授课语言", "Teaching language", [])}<small>{words(locale, "语言选项待提供。", "Language options are awaiting approval.")}</small>
